...
__global__ void medianFilterGPU(int* d_in, int* d_out, int histSize, int windSize) {
    ...
    // window length is 9. so it overflows by 1 on each side.
    __shared__ int smem[BLOCKSIZE+2][BLOCKSIZE+2];

    // populate shared memory from histogram (histogram is padded with zeros)
    if (is_x_top) {
        for (int i = 0; i < 1; i++) smem[1+ty][i] = d_in[(y+1) * (histSize+2) + (x+1)]; // d_in[(y+1) * (histSize+2) + (x+1-(i+1))];

    } else if (is_x_bot) {
        for (int i = 0; i < 1; i++) smem[1+ty][1+BLOCKSIZE+i] = d_in[(y+1) * (histSize+2) + (x+1)];
    }
    if (is_y_top) {
        for (int i = 0; i < 1; i++) smem[i][1+tx] = d_in[(y+1) * (histSize+2) + (x+1)];

        // corner cases
        if (is_x_top) {
            for (int i = 0; i < 1; i++) {
                for (int j = 0; j < 1; j++) {
                    smem[i][j] = d_in[(y+1) * (histSize+2) + (x+1)];
                }
            }
        }
            ...
            for (int i = 0; i < 1; i++) {
                for (int j = 0; j < 1; j++) {
                    smem[i][1+BLOCKSIZE+j] = d_in[(y+1) * (histSize+2) + (x+1)];
                }
            }
        }

    } else if (is_y_bot) {
        for (int i = 0; i < 1; i++) smem[1+BLOCKSIZE+i][1+tx] = d_in[(y+1) * (histSize+2) + (x+1)];

        // corner cases
        if (is_x_top) {
            for (int i = 0; i < 1; i++) {
                for (int j = 0; j < 1; j++) {
                    smem[1+BLOCKSIZE+i][j] = d_in[(y+1) * (histSize+2) + (x+1)];
                }
            }
        }
            ...
            for (int i = 0; i < 1; i++) {
                for (int j = 0; j < 1; j++) {
                    smem[1+BLOCKSIZE+i][1+BLOCKSIZE+j] = d_in[(y+1) * (histSize+2) + (x+1)];
                }
            }
        }
    }

    smem[1+ty][1+tx] = d_in[(y+1) * (histSize+windSize-1) + (x+1)];
    ...
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
            v[idx++] = smem[(ty+1) + i][(tx+1) + j];
        }
    }

    int med = getMedian(v, 3 * 3);
