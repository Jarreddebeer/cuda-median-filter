...
__global__ void medianFilterGPU(int* d_in, int* d_out, int histSize, int windSize) {
    ...
    // window length is 9. so it overflows by 10 on each side.
    __shared__ int smem[BLOCKSIZE+20][BLOCKSIZE+20];

    // populate shared memory from histogram (histogram is padded with zeros)
    if (is_x_top) {
        for (int i = 0; i < 10; i++) smem[10+ty][i] = d_in[(y+10) * (histSize+20) + (x+10)]; // d_in[(y+10) * (histSize+20) + (x+10-(i+1))];

    } else if (is_x_bot) {
        for (int i = 0; i < 10; i++) smem[10+ty][10+BLOCKSIZE+i] = d_in[(y+10) * (histSize+20) + (x+10)];
    }
    if (is_y_top) {
        for (int i = 0; i < 10; i++) smem[i][10+tx] = d_in[(y+10) * (histSize+20) + (x+10)];

        // corner cases
        if (is_x_top) {
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    smem[i][j] = d_in[(y+10) * (histSize+20) + (x+10)];
                }
            }
        }
            ...
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    smem[i][10+BLOCKSIZE+j] = d_in[(y+10) * (histSize+20) + (x+10)];
                }
            }
        }

    } else if (is_y_bot) {
        for (int i = 0; i < 10; i++) smem[10+BLOCKSIZE+i][10+tx] = d_in[(y+10) * (histSize+20) + (x+10)];

        // corner cases
        if (is_x_top) {
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    smem[10+BLOCKSIZE+i][j] = d_in[(y+10) * (histSize+20) + (x+10)];
                }
            }
        }
            ...
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    smem[10+BLOCKSIZE+i][10+BLOCKSIZE+j] = d_in[(y+10) * (histSize+20) + (x+10)];
                }
            }
        }
    }

    smem[10+ty][10+tx] = d_in[(y+10) * (histSize+windSize-1) + (x+10)];

    __syncthreads();

    // get window from shared memory
    int v[21 * 21 * sizeof(int)] = {};
    int idx = 0;
    for (int i = -10; i <= 10; i++) {
        for (int j = -10; j <= 10; j++) {
            v[idx++] = smem[(ty+10) + i][(tx+10) + j];
        }
    }

    int med = getMedian(v, 21 * 21);
...
int main(int argc, char **argv) {
    ...
    readHistogramCsvFile("grid-2048.csv", grid, histSize, windSize);
