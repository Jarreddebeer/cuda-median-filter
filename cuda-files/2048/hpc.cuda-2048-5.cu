...
__global__ void medianFilterGPU(int* d_in, int* d_out, int histSize, int windSize) {
    ...
    // window length is 9. so it overflows by 2 on each side.
    __shared__ int smem[BLOCKSIZE+4][BLOCKSIZE+4];

    // populate shared memory from histogram (histogram is padded with zeros)
    if (is_x_top) {
        for (int i = 0; i < 2; i++) smem[2+ty][i] = d_in[(y+2) * (histSize+4) + (x+2)]; // d_in[(y+2) * (histSize+4) + (x+2-(i+1))];

    } else if (is_x_bot) {
        for (int i = 0; i < 2; i++) smem[2+ty][2+BLOCKSIZE+i] = d_in[(y+2) * (histSize+4) + (x+2)];
    }
    if (is_y_top) {
        for (int i = 0; i < 2; i++) smem[i][2+tx] = d_in[(y+2) * (histSize+4) + (x+2)];

        // corner cases
        if (is_x_top) {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    smem[i][j] = d_in[(y+2) * (histSize+4) + (x+2)];
                }
            }
        }
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    smem[i][2+BLOCKSIZE+j] = d_in[(y+2) * (histSize+4) + (x+2)];
                }
            }
        }

    } else if (is_y_bot) {
        for (int i = 0; i < 2; i++) smem[2+BLOCKSIZE+i][2+tx] = d_in[(y+2) * (histSize+4) + (x+2)];

        // corner cases
        if (is_x_top) {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    smem[2+BLOCKSIZE+i][j] = d_in[(y+2) * (histSize+4) + (x+2)];
                }
            }
        }
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    smem[2+BLOCKSIZE+i][2+BLOCKSIZE+j] = d_in[(y+2) * (histSize+4) + (x+2)];
                }
            }
        }
    }

    smem[2+ty][2+tx] = d_in[(y+2) * (histSize+windSize-1) + (x+2)];

    __syncthreads();

    // get window from shared memory
    int v[5 * 5 * sizeof(int)] = {};
    int idx = 0;
    for (int i = -2; i <= 2; i++) {
        for (int j = -2; j <= 2; j++) {
            v[idx++] = smem[(ty+2) + i][(tx+2) + j];
        }
    }

    int med = getMedian(v, 5 * 5);
